 ...
 *   - chatStream(): SSE streaming — async-iterate text deltas, then await
 *     `.final` for requestId / usage / routedTo / tool calls
...
/** Final metadata for a completed stream — resolved via `chatStream(...).final`. */
export interface ChatStreamResult {
  /** Full concatenated assistant text. */
  content: string;
  /** Tool calls assembled from streamed deltas (empty if none). */
  ...
  execVerify: string | null;
  /** Token usage if the gateway sent a final usage frame. */
  usage: TokenUsage | null;
}

/**
 * A live chat stream. Async-iterate it to get text deltas, and await `.final`
 * afterwards for metadata (requestId, usage, routedTo, tool calls).
 *
 *   const stream = codai.chatStream({ messages });
 *   for await (const delta of stream) process.stdout.write(delta);
 *   const { requestId, usage } = await stream.final;
 */
export interface ChatStream extends AsyncIterable<string> {
  /** Resolves once the stream completes (or rejects if it errors). */
  ...
  for (const [key, val] of Object.entries(fragment)) {
    ...
    if (key === 'function' && val && typeof val === 'object') {
      ...
      for (const [fk, fv] of Object.entries(val as Record<string, unknown>)) {
        if (fk === 'arguments') {
          fn.arguments = String(fn.arguments ?? '') + String(fv ?? '');
        } else {
          fn[fk] = fv;
        }
      }
...
export class Codai {
  ...
  /**
   * Streaming chat. Async-iterate the returned object for text deltas, then
   * `await stream.final` for metadata (requestId, usage, routedTo, toolCalls).
   */
  chatStream(opts: ChatOptions): ChatStream {
    const requestPromise = this.request('/v1/chat/completions', this.chatBody(opts, true), {
      signal: opts.signal,
      headers: this.chatHeaders(opts),
    });
    ...
    // Avoid unhandled-rejection noise if the caller never awaits `final`.
    final.catch(() => {});

    // Accumulate tool-call fragments by index (OpenAI streams them piecewise).
    const toolCallsByIndex = new Map<number, Record<string, unknown>>();

    async function* iterate(this: Codai): AsyncGenerator<string, void, undefined> {
      let content = '';
      ...
      if (!reader) {
        const e = new CodaiError('no response body', res.status, null);
        rejectFinal(e);
        throw e;
      }
            ...
            for (const line of frame.split('\n')) {
              ...
              if (delta?.tool_calls) {
                for (const tc of delta.tool_calls) {
                  ...
                  mergeToolCall(toolCallsByIndex, idx, tc);
                }
              }
              if (delta?.content) {
